...
module Modes = struct
  ...
  let of_expr ({ pexp_desc; pexp_attributes; _ } as expr) =
    ...
    | _ ->
      Misc.fatal_errorf
        "Improperly encoded modes expression: %a"
  ...
  let expr_of ~loc (Coerce (modes, body)) =
    ...
    | Some payload ->
      let ext =
        Ast_helper.Exp.extension ~loc:modes.loc (Location.mknoloc extension_name, payload)
      in
    ...
    | Some ("mod", [ item_of_t; item_of_mode_expr ], loc) ->
      bind (of_structure_item item_of_t) (fun { txt = t } ->
        bind (struct_item_to_attr item_of_mode_expr) (fun attr ->
          let mode_list = Mode_expr.Const.list_from_payload ~loc attr.attr_payload in
